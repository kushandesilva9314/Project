...
export default function SalesTracker() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto bg-gray-200 min-h-screen">
      <img src={logo} alt="Logo" className="w-12 h-12 cursor-pointer mb-6" onClick={() => navigate("/company")} />
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-3 gap-4">
          {Array.from({ length: 24 }).map((_, i) => (
            <div key={i} className="flex flex-col items-center">
              <span className="text-sm font-semibold text-gray-600">{getMonthYearLabel(i)}</span>
              <input
                type="number"
                className="w-full p-3 border border-gray-300 rounded-lg bg-gray-100 text-lg text-center focus:outline-none appearance-none"
                value={sales[i] || ""}
                onChange={(e) => {
        ...
        <div className="text-center mt-6">
          ...
          <button onClick={() => {
            axios.delete("http://localhost:3001/api/sales/delete-sales", { withCredentials: true }).then(() => {
              setSubmitted(false);
      ...
      {!submitted && (
        <button onClick={() => {
          ...
          axios.post("http://localhost:3001/api/sales/add-sales", { sales: numericSales }, { withCredentials: true }).then(() => {
            setSubmitted(true);
          }).catch(error => {
      ...
      {submitted && (
        <div className="mt-6 text-center">
          ...
          <input
            type="number"
            className="w-full p-3 border border-gray-300 rounded-lg bg-gray-100 text-lg text-center focus:outline-none appearance-none mt-2"
          ...
          <button
            onClick={() => {
              ...
              axios.post("http://localhost:3001/api/sales/add-sales", { amount }, { withCredentials: true }).then(() => {
            ...
            className={`px-6 py-3 rounded-lg mt-4 w-full ${
              isButtonDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-green-500 text-white hover:bg-green-600 transition-all"
            }`} 
          >
            Add Sales
          </button>
